...
public class GameOver extends State {
    ...
    public GameOver(Manager m) {
        ...
        try {
            ...
            image = ImageIO.read(getClass().getResourceAsStream("Images/image.gif")).getSubimage(0, 0, 31, 34);

        }
        catch(Exception e) { //If loading fails
            e.printStackTrace();
            System.out.println("Error loading about GameOver graphics.");
            System.exit(0);
        }
    ...
    public void draw(Graphics2D g) {
        ...
        // draw title
        g.setFont(titleFont);
        g.drawString("Game Over", 85, 70);

        g.setFont(textFont);
        g.drawString("You died.", 135, 100);
